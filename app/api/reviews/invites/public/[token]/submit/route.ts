...
export async function POST(
  ...
  if (!INVITE_TOKEN_RE.test(token)) {
    return NextResponse.json({ error: "INVITE_INVALID", reason: "not_found" }, { status: 409 })
  }
  ...
  const { rating, body: reviewBody, treatment } = body as Record<string, unknown>
  ...
  if (
    ...
    reviewBody.trim().length > 800
  ) {
    return NextResponse.json(
      { error: "La reseña debe tener entre 20 y 800 caracteres." },
      { status: 400 }
    )
  }
  if (treatment !== undefined && treatment !== null && treatment !== "" && typeof treatment !== "string") {
    return NextResponse.json({ error: "Tratamiento inválido." }, { status: 400 })
  }
  if (typeof treatment === "string" && treatment.trim().length > 150) {
    return NextResponse.json({ error: "Tratamiento demasiado largo." }, { status: 400 })
  }

  const payload = {
    rating,
    body: reviewBody.trim().slice(0, 800),
    treatment:
      typeof treatment === "string" && treatment.trim() ? treatment.trim().slice(0, 150) : null,
  }
